...
def main():
    ...
    if hp == None:
        for aer in aer_list[:]:
            ...
            aa=gather_data(path1,aer,path1.strip().split('/')[-3],model,plev=pv,reg=region)
            bb=gather_data(path2,aer,path2.strip().split('/')[-3],model,plev=pv,reg=region)
            aa[0].load()
            bb[0].load()
            aa[1].load()
            bb[1].load()
    ...
    if vl != None:
        ...
        aa=gather_data(path1,vlist,path1.strip().split('/')[-3],model,sv='y')
        bb=gather_data(path2,vlist,path2.strip().split('/')[-3],model,sv='y')
        aa[0].load()
        bb[0].load()
        aa[1].load()
        bb[1].load()
